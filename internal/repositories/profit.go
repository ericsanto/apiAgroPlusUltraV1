...
func (p *ProfitRepository) FindProfit(batchID, farmID, userID, plantingID uint) (*responses.ProfitResponse, error) {
	...
	WHERE planting_entities.id = ? AND user_models.id = ? AND farm_entities.id = ?
	GROUP BY farm_entities.id, batch_entities.id, user_models.id`

	var responseProfit responses.ProfitResponse

	if err := p.db.Raw(query, plantingID, userID, plantingID).Scan(&responseProfit).Error; err != nil {
